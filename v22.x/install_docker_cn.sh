...
# Usage:    sudo /bin/sh install_docker_cn.sh
# ============================================================
...
if [ "$(id -u)" -ne 0 ]; then
    ...
    echo "        sudo /bin/sh install_docker_cn.sh"
    exit 1
fi
...
if ! curl -s --connect-timeout 5 https://mirrors.aliyun.com > /dev/null 2>&1; then
    log_warn "Cannot reach mirrors.aliyun.com — will try to proceed anyway."
fi
...
echo "deb [arch=amd64 signed-by=/etc/apt/keyrings/docker.gpg] https://mirrors.aliyun.com/docker-ce/linux/${ID} ${CODENAME} stable" | \
