...
start_extension(){
    ...
    export_configure_crt_or_up
    set +e
    docker compose -f ${im_compose_ini_file} down -v > /dev/null || true
    docker compose -f ${im_compose_ini_file} up -d --wait > /dev/null
    local crtOrUpRetEnv=$?
...
EOF
    ...
    # run extend service
    docker compose -f ${im_compose_file} up -d > /dev/null
